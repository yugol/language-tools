...
public class Nome extends ParteDelDiscorso {
	...
	static Genere identifyGenere(String lemma, Genere genere) {
		if (genere == null) {
			...
			if (StringUtil.isVowel(last)) {
				...
				case 'e':
					...
					if (lemma.endsWith("ore") || lemma.endsWith("one")) {
						return Genere.MASCHILE;
					}
				}
			} else {
	...
	static String identifyPlurale(String lemma, Genere genere, String plurale) {
		if (StringUtil.isNullOrBlank(lemma)) {
			return "";
		}
		genere = identifyGenere(lemma, genere);
		if (plurale == null) {
			char last = lemma.charAt(lemma.length() - 1);
			if (last == 'à' || lemma.endsWith("a`")) {
				return lemma;
			...
			case MASCHILE:
				...
				if (lemma.endsWith("co") || lemma.endsWith("ca")) {
					return lemma.substring(0, lemma.length() - 2) + "chi";
				}
				if (lemma.endsWith("go") || lemma.endsWith("ga")) {
